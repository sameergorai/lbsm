...
function Page() {
    ...
    const titleMap = {
        ...
        'users': 'User Management',
        'carousel': 'Carousel Management',
        'gallery': 'Gallery Management', // Added Gallery Title
        'FACULTIES ': 'Faculty Management',
        'seminar': 'Seminar Management',
        'sport': 'Sports Management',
        'notice': 'Notice Board',
    ...
    const navItems = [
        ...
        { id: 'users', icon: 'fa-users', label: 'Users' },
        { id: 'carousel', icon: 'fa-images', label: 'Carousel' },
        { id: 'gallery', icon: 'fa-camera', label: 'Gallery' }, // Added Gallery Nav Item
        { id: 'FACULTIES ', icon: 'fa-file-pdf', label: 'Faculty' },
        { id: 'seminar', icon: 'fa-chalkboard-teacher', label: 'Seminar' },
        { id: 'sport', icon: 'fa-running', label: 'Sport' },
        { id: 'notice', icon: 'fa-clipboard', label: 'Notice Board' },
    ...
    return (
        <div className="bg-gray-100 font-sans antialiased">
            <div className="flex h-screen overflow-hidden">
                ...
                {/* Sidebar */}
                <aside className="w-64 bg-white text-gray-500 font-bold flex-shrink-0 hidden md:flex flex-col transition-all duration-300">
                    <div className="h-16 flex items-center justify-center border-b border-slate-700">
                        <h1 className="text-2xl font-bold tracking-wider">ADMIN<span className="text-blue-500">PANEL</span></h1>
                    </div>
                        ...
                        <ul className="space-y-1">
                            {navItems.map((item) => (
                                <li key={item.id}>
                                    <button
                                        onClick={() => setActiveSection(item.id)}
                                        className={`nav-btn w-full text-left px-9 py-3 hover:bg-slate-300 transition-colors flex items-center border-l-4 focus:outline-none text-xl rounded-2xl shadow my-2 ${activeSection === item.id
                                            ? 'bg-slate-800 border-blue-500'
                                            : 'border-transparent'
                                            }`}
                                    >
